...
public enum ChatColors {
  ...
  public static ChatColors getClosestColor(java.awt.Color target) {
    ...
    for (ChatColors color : ChatColors.values()) {
      if (color == ChatColors.MAGIC
          || color == ChatColors.BOLD
          || color == ChatColors.STRIKETHROUGH
          || color == ChatColors.UNDERLINE
          || color == ChatColors.ITALIC
          || color == ChatColors.RESET) {
        continue;
      }
      java.awt.Color c = new java.awt.Color(color.toAwtColor());
      double dist =
          Math.pow(c.getRed() - target.getRed(), 2)
