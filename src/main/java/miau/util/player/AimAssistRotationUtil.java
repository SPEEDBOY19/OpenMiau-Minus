...
/**
 * Ported from FB's AimAssistRotationUtil.
 */
public class AimAssistRotationUtil {
  ...
  public static double[] heuristics(Entity entity, double[] xyz) {
    ...
    double minX =
        MathHelper.clamp_double(
            xyz[0] - boxSize,
            entity.getEntityBoundingBox().minX - (double) f11,
            entity.getEntityBoundingBox().maxX + (double) f11);
    double minY =
        MathHelper.clamp_double(
            xyz[1] - boxSize,
            entity.getEntityBoundingBox().minY - (double) f11,
            entity.getEntityBoundingBox().maxY + (double) f11);
    double minZ =
        MathHelper.clamp_double(
            xyz[2] - boxSize,
            entity.getEntityBoundingBox().minZ - (double) f11,
  ...
  public static float randomSin() {
    return MathHelper.sin(getInRange(0F, (float) (2F * Math.PI)));
  }
  ...
  public static float[] face(
    ...
    float[] origin = getRotations(mc.thePlayer.getPositionEyes(1.0f), point);
    ...
    calcYaw +=
        randomizeStrength
            * (getInRange(-1, 1)
                + getInRange(-1, 1) * getInRange(0, 1)
                + getInRange(-1, 1) * getInRange(0, 1) * getInRange(0, 1)
                + getInRange(-1, 1)
                    * getInRange(0, 1)
                    * getInRange(0, 1)
                    * getInRange(0, 1)
        ...
        (float)
            ((double) calcPitch
                + 1.356526526
                    * Math.sin(
                        0.06981317007977318
                            * (double) (updateRotation(currentYaw, calcYaw, 180.0F) - calcYaw))
                    * 8.0);
    ...
    int deltaY =
        (int) ((6.667 * (double) calcPitch - 6.667 * (double) currentPitch) / (double) f2) * -1;
  ...
  public static float[] face(
    ...
    float[] current = new float[] {mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch};
    float[] origin = getRotations(mc.thePlayer.getPositionEyes(1.0f), targetPos);
    float[] target = new float[] {origin[0], origin[1]};
    float[] finalR =
        smoothRotation(target, current, yawSpeed, pitchSpeed, entity, P1Max, P1Min, entropyFactor);
    calcYaw = finalR[0];
    calcPitch = finalR[1];
    calcYaw +=
        randomizeStrength
  ...
  public static float[] smoothRotation(
    ...
    float smoothFactor =
        MathHelper.clamp_float(
            (float)
                (1.0
                    - mc.thePlayer.getDistance(point.xCoord, point.yCoord, point.zCoord) / 15.0),
            0.3F,
            0.9F);
    ...
    float deltaYawDiff =
        MathHelper.wrapAngleTo180_float(targetRotations[0] - currentRotations[0]);
    ...
    float yaw =
        invT * invT * invT * currentRotations[0]
    ...
    float pitch =
        invT * invT * invT * currentRotations[1]
    ...
    yaw =
        currentRotations[0]
            + MathHelper.clamp_float(
                MathHelper.wrapAngleTo180_float(yaw - currentRotations[0]), -deltaYaw, deltaYaw);
    pitch =
        currentRotations[1]
            + MathHelper.clamp_float(pitch - currentRotations[1], -deltaPitch, deltaPitch);
  ...
  public static float[] smoothRotation(
    ...
    float smoothFactor =
        MathHelper.clamp_float(
            (float)
                (1.0
                    - mc.thePlayer.getDistanceToEntity(target) / 15.0
                    + speed * 0.3),
            0.3F,
            0.9F);
    ...
    targetRotations =
        new float[] {
          targetRotations[0] + (float) perturbationYaw,
          targetRotations[1] + (float) perturbationPitch
        };
